...
if (empty($_POST['id'])) {
    // Insert new news
    $query = "INSERT INTO news (user_ID, news_detail, news_img) VALUES ('{$user_ID}', '{$news_detail}', '{$news_img}')";
    ...
    $query = "UPDATE news SET user_ID='{$user_ID}', news_detail='{$news_detail}', news_img='{$news_img}' WHERE news_ID='{$news_ID}'";
    $result = mysqli_query($conn, $query) or die('Query failed: ' . mysqli_error($conn));
}
