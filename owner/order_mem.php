        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="manageModalLabel">รายละเอียดคำสั่งซื้อ</h5>
                ...
                <div class="card card-lg" style="width: 80rem;">
                    <div class="card-header">
                        <div class="btn-group" role="group" aria-label="Order Status Filters">
                            <button type="button" class="btn btn-text-dark" onclick="showStatus('not_paid')">ยังไม่ได้ชำระ (<?php echo $paymentCounts['not_paid']; ?>)</button>
                            <button type="button" class="btn btn-text-dark" onclick="showStatus('paid')">ชำระแล้ว (<?php echo $paymentCounts['paid']; ?>)</button>
                            
                            <button type="button" class="btn btn-text-dark" onclick="showStatus('preparing')">เตรียมจัดของ (<?php echo $statusCounts['preparing']; ?>)</button>
                            <button type="button" class="btn btn-text-dark" onclick="showStatus('ready_to_ship')">เตรียมส่ง (<?php echo $statusCounts['ready_to_ship']; ?>)</button>
                            <button type="button" class="btn btn-text-dark" onclick="showStatus('shipped')">จัดส่ง (<?php echo $statusCounts['shipped']; ?>)</button>
                            <button type="button" class="btn btn-text-dark" onclick="showStatus('completed')">เสร็จสิ้น (<?php echo $statusCounts['completed']; ?>)</button>
                        </div>
                    </div>
                    <div class="card-body">
                        <div id="order-info">
            ...
            const orderRows = filteredOrders
                .map(order => `
                    <tr>
                        ...
                        <td>
                            <button type="button" class="btn btn-outline-dark" data-bs-toggle="modal" data-bs-target="#manageModal" onclick="loadOrderDetails(${order.or_ID})">จัดการ</button>
                            ${status === 'not_paid' ? `<a onclick="return confirm('Are you sure you want to delete?');" href="<?php echo $base_url; ?>/owner/order_delete.php?id=${order.or_ID}" class="btn btn-outline-danger">ลบ</a>` : ''}
                        </td>
                    </tr>
                `)
                .join('');
